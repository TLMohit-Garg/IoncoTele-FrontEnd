...
export default function SearchDoctor() {
    ...
    const handleSelect = (event: React.SyntheticEvent, doctor: DoctorType | null) => {
        if (doctor) {
            console.log("Selected doctor object:", doctor); // Logs the entire selected object
            if (doctor.id) {
  ...
  return (
    <Autocomplete
      ...
      autoHighlight
      getOptionLabel={(option: DoctorType) => `${option.title} - ${option.speciality}`}
      onChange={handleSelect}
      filterOptions={(options, state) => {
        // Custom filter to search both by name and speciality
        const query = state.inputValue.toLowerCase();
        return options.filter((option) =>
          option.title.toLowerCase().includes(query) || option.speciality.toLowerCase().includes(query)
        );
      }}
